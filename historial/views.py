...
def crear_historia(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            model_doctor = request.user.content_type.model_class()
            doctor = model_doctor.objects.get(id=request.user.role_data_id)
            historia_clinica.doctor = doctor
            historia_clinica.save()
            
            
            messages.success(request, 'Historia Clinica guardada con exito')
            return redirect('ver_historia_clinica', historia_clinica_id=historia_clinica.id)
        else:
    ...
    return render(request, 'registrar_historia_clinica.html', {'form': form})
...
def ver_historia_clinica(request, historia_clinica_id):
    ...
    #     return redirect('doctores/login.html')
